...
Public Class GWOGraphicsForm
    Private Sub GWOGraphicsForm_Load(sender As Object, e As EventArgs) Handles Me.Load
...
#Region "Subs/Functions"
    Private Sub ShowData()
        Try
            If IsNothing(MainModule.GWO_Engine) = False Then

                ShowDataGraphics(GWO_Engine.Release, GWO_Engine.Demands, Chart1)
                ShowDataGraphics(GWO_Engine.Storage, Chart2)
                ShowDataGraphics(GWO_Engine.OverFlow, Chart3)

                If IsNothing(GWO_Engine.Best_Charts) = False Then
                    CmbBestCharts.Items.Clear()
                    For Each serie In GWO_Engine.Best_Charts
...
#End Region
    ...
    Private Sub CmbBestCharts_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbBestCharts.SelectedIndexChanged
        ...
        If index >= 0 Then
            ShowDataGraphics(GWO_Engine.Best_Charts(index), Chart4)
        End If
    End Sub

    Private Sub TsmiRefreshChart1_Click(sender As Object, e As EventArgs) Handles TsmiRefreshChart1.Click
        Try
            If IsNothing(GA_Engine) Then Return
            If IsNothing(GA_Engine.Release) Then Return
            If IsNothing(GA_Engine.Demands) Then Return

            ShowDataGraphics(GA_Engine.Release, GA_Engine.Demands, Chart1)
