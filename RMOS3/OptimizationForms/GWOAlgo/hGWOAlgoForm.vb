...
Public Class bridPSOGWOAlgoForm
    ...
    Private Sub GWOAlgoForm_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Try
            ...
            If IsNothing(MainModule.List_InflowQSeries) = False Then
                For Each ds1 In MainModule.List_InflowQSeries
                    CmbInflowSeries.Items.Add(ds1.Name)
                Next
            End If

            If IsNothing(MainModule.List_DemandsSeries) = False Then
                For Each ds1 In MainModule.List_DemandsSeries
                    CmbDemands.Items.Add(ds1.Name)
                Next
            End If
    ...
    Private Sub BtnLuanchOptimization_Click(sender As Object, e As EventArgs) Handles BtnLuanchOptimization.Click
        Me.Cursor = Cursors.WaitCursor
        Try
            BtnLuanchOptimization.Text = "<||>"

            With GWO_Engine
                .Reservoir = TheReservoir
                .InflowSerie = MainModule.List_InflowQSeries(Index_InflowSerie)
                .Demands = MainModule.List_DemandsSeries(Index_DemandsSerie)
                '--------------------------------------------------
                .Luanch_Optimization()
                '--------------------------------------------------
            End With
...
#End Region
    ...
    Private Sub CxtMiShowGraphicsForm_Click(sender As Object, e As EventArgs) Handles CxtMiShowGraphicsForm.Click
        Dim graphicsFrm = New GraphicsForm()
        graphicsFrm.OptimizationEngine = GWO_Engine
        MainForm.Show_Window(CType(graphicsFrm, Form))
    End Sub

    Private Sub GWOAlgoForm_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
